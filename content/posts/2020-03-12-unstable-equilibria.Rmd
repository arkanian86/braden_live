...
y1 <- distribution_normal(n = 100, mean = 0, sd = 1)
y2 <- distribution_normal(n = 100, mean = 2, sd = 1)
# x <- seq(-4, 6, .1)

y1 <- as.data.frame(density(y1))
y2 <- as.data.frame(density(y2))


two_curves <- full_join(y1, y2, by = "x")
...
ggplot(data) +
  # geom_line(aes(x=x, y=y), size=1) +
  # geom_line(aes(x=x, y=e), size=1) +
  geom_vline(aes(xintercept=x_intercept), size=1) +
  # geom_vline(aes(xintercept=.6516307)) +
  # geom_hline(aes(yintercept=0), size=1) +
  geom_ribbon(data = subset(data, x<=x_intercept), aes(x=x, ymin=0, ymax=y), fill = "blue", alpha = .3) +
  geom_ribbon(data = subset(data, e<y & x<x_intercept), aes(x=x, ymin=0, ymax=e), fill = "light blue", alpha = .5) +
  geom_ribbon(data = subset(data, y>e & x>x_intercept), aes(x=x, ymin=e, ymax=y), fill = "pink", alpha = .3) +
  geom_ribbon(data = subset(data, e<y & x>=x_intercept | x==data[306,1]), aes(x=x, ymin=0, ymax=e), fill = "purple", alpha = .3) +
  geom_ribbon(data = subset(data, y<=e & x>=x_intercept), aes(x=x, ymin=0, ymax=y), fill = "purple", alpha = .3) +
  geom_ribbon(data = subset(data, e>y & x>=x_intercept), aes(x=x, ymin=y, ymax=e), fill = "red", alpha = .3) +
  theme(axis.line=element_blank(),
      axis.text.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks=element_blank(),
...
# filter(data, y==e)
# tail(filter(data,e<=y & x>=x_intercept))
```
...
# x <- seq(-2, 6, .01)
# y2 <- dnorm(x2, mean=2)
# xy2 <- as.data.frame(cbind(x,y2))
# 
# all <- full_join(xy, xy2, by = "x")
# 
# 
# ggplot(all) +
...
#   # geom_vline(aes(xintercept=x_intercept), size=1) +
#   # geom_hline(aes(yintercept=0), size=1) +
#   geom_ribbon(data = subset(all, x<=x_intercept), aes(x=x, ymin=0, ymax=y), fill = "blue", alpha = .3) +
#   geom_ribbon(data = subset(all, y2<y & x<x_intercept), aes(x=x, ymin=0, ymax=y2), fill = "light blue", alpha = .5) +
#   geom_ribbon(data = subset(all, y>y2 & x>x_intercept), aes(x=x, ymin=y2, ymax=y), fill = "pink", alpha = .3) +
#   geom_ribbon(data = subset(all, y2<y & x>=x_intercept), aes(x=x, ymin=0, ymax=y2), fill = "purple", alpha = .3) +
#   geom_ribbon(data = subset(all, y<=y2 & x>=x_intercept), aes(x=x, ymin=0, ymax=y), fill = "purple", alpha = .3) +
#   geom_ribbon(data = subset(all, y2>y & x>=x_intercept), aes(x=x, ymin=y, ymax=y2), fill = "red", alpha = .3) +
#   theme(axis.line=element_blank(),
#       axis.text.x=element_blank(),
#       axis.text.y=element_blank(),
#       axis.ticks=element_blank(),
...
```{r}
foo <- distribution_normal(n = 100, mean = 0, sd = 1) %>%
  density() %>%
  as.data.frame()

foo2 <- distribution_normal(n = 100, mean = 2, sd = 2) %>%
  density() %>%
  as.data.frame()

all_foo <- cbind(foo, foo2)

colnames(all_foo) <- c('x1', 'y1', 'x2', 'y2')

ggplot(all_foo) +
  geom_line(aes(x=x1, y=y1)) +
  geom_line(aes(x=x2,y=y2)) +
  geom_ribbon(aes(x=0, ymin=y2, ymax=y1))
```
...
ggplot(foo) +
  ...
  geom_line(aes(x=x, y=y)) +
  geom_line(aes(x=x, y=y2)) +
  geom_ribbon(data=subset(foo, y>y2 & x <  0), aes(x=x, ymin=y2, ymax=y), fill = "blue", alpha = .3) +
  geom_ribbon(data = subset(foo, y2<y & x< 0), aes(x=x, ymin=0, ymax=y2), fill = "light blue", alpha = .5) +
  geom_ribbon(data = subset(foo, y>y2 & x> 0), aes(x=x, ymin=y2, ymax=y), fill = "pink", alpha = .3) +
  geom_ribbon(data = subset(foo, y2<y & x>=0), aes(x=x, ymin=0, ymax=y2), fill = "purple", alpha = .3) +
  geom_ribbon(data = subset(foo, y<=y2 & x>0), aes(x=x, ymin=0, ymax=y), fill = "purple", alpha = .3) +
  geom_ribbon(data = subset(foo, y2>y & x>=x_intercept), aes(x=x, ymin=y, ymax=y2), fill = "red", alpha = .3) +
  xlim(-5,5)
```
