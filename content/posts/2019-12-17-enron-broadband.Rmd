...
EBS_markets <- summarize(top_cities, TAM=sum(pop_2010), SAM=.32*TAM, SOM=.70*SAM, VoD_subs=SOM/.50)
...
NFLX_markets <- summarize(US_pop_2010, TAM=sum(pop), SAM=perc_dsl_access*TAM, SOM=20010000, VoD_subs=SAM*.32)
...
ggplot(provider_markets, aes(variable, value)) + 
  geom_bar(stat='identity', aes(fill = provider), width=.75, position="dodge") +
  coord_flip() +
  theme_new() + scale_fill_manual(values=c("#0084C7","#708088")) +
  ggtitle("Netflix better predicted the market") +
  theme(
...
The U.S. population in 2010 was `r format(US_pop_2010, big.mark=",")`, so Netflix



https://www.geek.com/gadgets/netflix-gains-7-7m-subscribers-in-2010-thanks-to-streaming-only-plan-1354915/
https://techcrunch.com/2010/07/21/netflix-users/
